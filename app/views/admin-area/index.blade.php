...
@endforeach
	...
	<thead>
		<tr >
			<th style="border:#023332 thin solid"  >sn</th>
                        <th style="border:#023332 thin solid" >Username</th>
                        <th width='10%' style="border:#023332 thin solid" ></th>
			<th  width='5%' style="border:#023332 thin solid" ></th>
  ...
  @foreach($row as $key => $news)
  		<tr class="odd gradeX">
			...
			<td style="border:#023332 thin solid" >{{$news->username }}</td>
                       <td style="border:#023332 thin solid" >
     <div class="btn-group">
   <button type="button" class="btn btn-warning dropdown-toggle" data-toggle="dropdown">
    Previledge <span class="caret"></span>
   </button>                       
     <ul class="dropdown-menu" role="menu">

         <li><a style="text-decoration: none" onclick=" ReAssign(<?php echo $news->id ;?>,0)" >Suspend  <?php echo ($news->access == 0 ) ? "<i class='fa fa-check'></i>" : ""; ?></a></li>
    <li><a style="text-decoration: none" onclick="ReAssign(<?php echo $news->id ;?>,1)" >User  <?php echo ($news->access == 1 ) ? "<i class='fa fa-check'></i>" : ""; ?></a></li>
    <li><a style="text-decoration: none" onclick=" ReAssign(<?php echo $news->id ;?>,2)">Admin<?php echo ($news->access == 2 ) ? "<i class='fa fa-check'></i>" : ""; ?></a></li>

   </ul>                           
                        </td>
			<td style="border:#023332 thin solid" >               <!-- delete the nerd (uses the destroy method DESTROY /nerds/{id} -->
    <!-- we will add this later since its a little more complicated than the other two buttons -->
{{ Form::open(array('url' => 'admin-area/' . $news->id, 'class' => 'pull-right ')) }}
{{ Form::hidden('_method', 'DELETE') }}
<!--            {{ Form::submit('X', array('class' => 'fa fa-home btn btn-warning ')) }}-->
<button type="submit" class="btn btn-danger "
onclick="javascript:return confirm('Are you sure to delete <?php echo $news->username ?>')" >  <i class="fa fa-trash-o"></i>
</button>
{{ Form::close() }}  
...
<!--
           ...
           -->
           {{-- HTML::link('admin-area/'.$news->id.'/edit-shoutout','',array('class'=>'btn btn-warning fa fa-edit'))--}}
