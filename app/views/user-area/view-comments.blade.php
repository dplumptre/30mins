<?php  $users = Auth::user();
$row = Comment::with('users')->where('user_id','=', $users->id )->orderBy('id', 'DESC')->get();
?>
...
@endforeach
	...
	<thead>
		<tr >
			...
			<th style="border:#023332 thin solid" >Comment</th>
  ...
  @foreach($row as $key => $news)
  		<tr class="odd gradeX">
			<td style="border:#023332 thin solid" >{{ $key +1 }}</td>
                        	<td style="border:#023332 thin solid" >{{$news->users->username }}</td>
...
{{ Form::open(array('url' => 'user-area/view-comments/' . $news->id, 'class' => 'pull-right ')) }}
{{ Form::hidden('_method', 'DELETE') }}
<!--            {{ Form::submit('X', array('class' => 'fa fa-home btn btn-warning ')) }}-->
<button type="submit" class="btn btn-danger "
onclick="javascript:return confirm('Are you sure to delete the Comment')" >  <i class="fa fa-trash-o"></i>
</button>
{{ Form::close() }}  
...
<!--
           ...
           -->
           {{ HTML::link('user-area/view-comments/'.$news->id.'/edit-comments','',array('class'=>'btn btn-warning fa fa-edit'))}}
